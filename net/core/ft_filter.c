...
static unsigned int ft_hook_after_network_layer_secondary_tcp(struct net_filter_info *filter, struct sk_buff *skb){
	...
	spin_unlock_bh(&filter->lock);
        ...
        filter_id_printed= print_filter_id(filter);
        FTPRINTK("%s: pid %d ft_pid %s tx packet %llu csum %d in filter %s\n", __func__, current->pid, ft_pid_printed, pckt_id, 0/*csum*/, filter_id_printed);
	if(ft_pid_printed)
...
static int try_compleate_handshake_seq_from_dispatcher(struct rx_copy_msg *msg){
	...
	if( !(listen_filter->type & FT_FILTER_ENABLE) 
	    || !(!(listen_filter->type & FT_FILTER_FAKE) && listen_filter->ft_sock && listen_filter->ft_sock->sk_state==TCP_LISTEN)) {
		unsigned long lf_type = listen_filter->type;
		spin_unlock_bh(&listen_filter->lock);
		put_ft_filter(listen_filter);
		printk("WARING %s: filter not listening state E:%lx F:%lx id:%lx sock:%lx state:%lx\n", __func__,
			(lf_type & FT_FILTER_ENABLE), (lf_type & FT_FILTER_FAKE), msg->filter_id, listen_filter->ft_sock, listen_filter->ft_sock ? listen_filter->ft_sock->sk_state: -1);
		return -1;
	}
	spin_unlock_bh(&listen_filter->lock);
