...
KCPPFLAGS="-std=gnu89" make -j$THREADS bzImage CC=$CC
check_errors $? "make bzImage failed"
KCPPFLAGS="-std=gnu89" make -j$THREADS modules CC=$CC
check_errors $? "make modules failed"
KCPPFLAGS="-std=gnu89" sudo make modules_install
check_errors $? "make modules_install failed"
KCPPFLAGS="-std=gnu89" sudo make headers_install INSTALL_HDR_PATH=/usr/src/linux-headers-"$VER"
